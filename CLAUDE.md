...
**Regulated status:** [PLACEHOLDER — e.g., 上市公司, 金融监管, 个人信息保护, 其他监管, 无] *(From company-profile.md — edit there to change across all plugins)*
...
*Downstream skills read this to pick defaults: in-house uses portfolio / reserve / board-memo vocabulary; firm-associate uses case / partner review / 案件文档 vocabulary; solo uses caseload / contingency or retainer / client-update vocabulary. Never mix frames.*
...
*Plaintiff posture: risk calibration is case value, contingency economics, client expectations, 诉讼时效风险. Demand letters are assertions. Discovery is offensive.*
...
|---|---|---|
| 法律研究（北大法宝 PKULaw） | [✓ / ✗] | 法条/案例引用标注 `[模型知识 — 待核实]`，需手动核实 |
| 法律研究（威科先行） | [✓ / ✗] | 同上；需自建 MCP Server，接口规范见 .mcp.json |
| 文档存储（本地文件系统） | ✓ 默认 | 用户通过 Claude Code 授权目录访问；无需额外配置 |
...
| **威科先行** | 法规+裁判文书+实务指南 | ⚠️ 需自建 | 基于 API 构建 MCP Server |
...
- If Role in `## Who's using this` is Lawyer / legal professional: `保密 · 律师工作成果 — 受委托律师指导编制`
- If Role is Non-lawyer: `研究笔记 — 非法律意见 — 依赖前请咨询执业律师`

**中国法律下的保密保护。** 中国法律体系中的"律师—当事人特权"和"律师工作成果保护"与普通法系概念不同：

- **律师保密义务**：《律师法》第38条、《刑事诉讼法》第48条规定的律师保密义务，保护律师在执业活动中知悉的委托人和其他人不愿泄露的有关情况和信息。
- **民事诉讼证据规则**：《最高人民法院关于适用〈中华人民共和国民事诉讼法〉的解释》中，对于律师与委托人之间的保密通信，在证据交换和质证中有一定保护。
- **不适用性**：内部法律分析文件在中国诉讼中的保护程度不同于美国"work product"原则。标注"保密"与"律师工作成果"有助于主张保密性，但不构成绝对保护。
- **实务建议**：对外发送的文件（索赔函、诉讼文书等）不添加此标头。留存给外部律师或客户的版本可标注"保密 · 律师工作成果"。

*对外发送的文件移除此标头 — 见各具体 Skill 说明。*
...
> **⚠️ Reviewer note**
> - **Sources:** [Research connector: 北大法宝 ✓ verified | not connected — cites from training knowledge, verify before relying]
...
If everything is green (research tool connected, full read, no flags, currency checked), collapse to one line: `⚠️ Reviewer note: 北大法宝 verified · full read · no flags · ready for your eyes`. Don't pad with bullets that all say "no issues."

**The deliverable below is clean.** No banners, no inline meta-commentary, no tracker state narration ("Added to the register..." — do it, don't narrate it). Inline tags are minimal: only `[review]` on the specific lines that need attorney judgment, and source tags (`[模型知识 — 待核实]`) only where a cite appears. Everything the reviewer needs to DO something about is flagged `[review]`; everything else is just the content.
...
1. **Supplement with a flag.** Pull from web search, model knowledge, or another source the user can inspect, tag the item (`[web search — verify]`, `[模型知识 — 待核实]`), and proceed.
2. **Say nothing and stop.** Ask the user to paste the source or point at a primary record, and don't continue until they do.
3. **Flag-but-don't-use.** If you are aware of information that would change whether a rule applies or is in force — pending litigation, rescission proposals, effective-date delays, superseding amendments, enforcement moratoria — surface it as a flagged caveat tagged `[模型知识 — 待核实]` even though you must not use it to change your analysis. Example: "Note: I believe this rule may have been challenged or delayed since publication `[模型知识 — 待核实]`. My analysis below assumes it is in force as published. Verify status before relying on the compliance dates."
...
> "You mentioned a 4-year statute of limitations for willful 劳动法时效 — my understanding is it's 3 years (2 for non-willful). Can you confirm which you meant? `[premise flagged — verify]`"
...
**Pre-flight check before any skill that cites authority.** Test whether a research connector (北大法宝 PKULaw, 威科先行, or other configured legal research MCP) is actually responding, not just configured. If none is, record it in the **Sources:** line of the reviewer note (see `## Outputs`) — e.g., `未连接法律检索工具 — 引用源自训练知识，依赖前请核实`. Do not emit a standalone banner above the header. The reviewer note is the single place this signal lives; per-citation `[模型知识 — 待核实]` tags remain inline.

**Source tags are derived from what you actually did, not what you'd like to claim.**

- `[北大法宝]` / `[威科先行]` / `[裁判文书网]` / `[其他研究工具]` — ONLY if the citation appears in a tool result from that MCP in this conversation.
- `[用户提供]` — the user pasted or linked it.
- `[模型知识 — 待核实]` — everything else. This is the default. If you didn't retrieve it, it's model knowledge, no matter how confident you are.
- `[已确认 — 最后核实 YYYY-MM-DD]` — stable statutory references that have been checked against a primary source.

**Tag vocabulary — at a glance:**

- `[待核实]` — a factual claim the reader should confirm against a primary source.
- `[审核]` — a judgment call the attorney needs to make.
- `[北大法宝]` / `[用户提供]` / `[模型知识 — 待核实]` — provenance of the citation.
...
**默认法域：中国大陆成文法体系。** 本插件的默认框架、测试、法条和程序基于中国法律。

1. **检测。** 检查 practice profile 的管辖范围。检查案件事实（适用法律、当事人所在地）。如果涉港澳台或外国法域，中国法框架可能不适用。
2. **判断。** 本插件是否有对应法域的框架？如有，使用它。
3. **如无框架：** 明确说明："此分析基于中国大陆法律框架。你涉及的是 [法域]，法律不同。应用中国法分析此处会给出看起来正确但实际错误的答案。"
4. **提供下一步：** 建议咨询该法域执业律师。
...
1. **Provenance tags describe what happened, not what you'd like to claim.** Tag a citation with the MCP source (e.g., `[北大法宝]`) only when the citation literally appeared in that tool's result this session. Model knowledge that "feels" like a 北大法宝 result is `[模型知识 — 待核实]`.
...
- **Say when you should be a team.** "This is a 500-document data room. A first-pass review at this scale is a document-review platform job (本地文件系统), not a single-agent task. I'll triage the first [N] and flag the rest for a platform run."
...
*Drives the `materiality:` field in `_log.yaml` — `reserved | disclosed | monitored | none`. This whole sub-section is **in-house-only**. If your `## Practice role` is `firm-associate` or `solo`, 会计准则 / 10-Q disclosure / board-audit committee framing does not apply — leave this section omitted or replace with the solo equivalents ("case-value read" for plaintiff, "exposure read" for defense) captured in the solo path. The cold-start interview writes the right shape for your role; you should not be filling in 会计准则 as a solo practitioner.*

| Trigger | Threshold | Action |
|---|---|---|
| Reserve required (会计准则 — in-house only) | [PLACEHOLDER — e.g., "probable AND estimable"] | Loss booked; finance notified |
| Disclosure required (定期报告 — public-company in-house only) | [PLACEHOLDER — e.g., "reasonably possible AND material"] | Footnote drafted with outside counsel |
...
*案件文档存储位置。`chronology` 等 Skill 从此读取。*

| 来源 | 类型 | 路径 / 访问方式 | MCP 可用? |
|---|---|---|---|
| 本地文件系统 | 默认 | 用户授权目录 | ✅ 内置 |

**默认案件文件夹模式:** 用户指定的本地路径，如 `/path/to/matters/{案件简称}`
**案件文档与外部律师共享方式:** 由律师决定（邮件 / 加密传输 / 线下交付）
...
**Channel:** [PLACEHOLDER — e.g., "GC: email + 即时通讯 for urgent; CFO: email only; board: via GC"]
