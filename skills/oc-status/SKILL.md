---
name: oc-status
description: 生成发送给外部律师的每周状态请求邮件草稿 — Markdown 格式，覆盖活跃案件组合。使用场景：用户要求外部律师状态请求、每周外部律师跟进、或从案件日志起草逐案状态邮件。
argument-hint: "[--all | --slug=foo]"
---

# /oc-status

1. 加载 `~/.claude/plugins/config/claude-for-legal/litigation-legal/matters/_log.yaml`，按规则过滤。
2. 加载 `~/.claude/plugins/config/claude-for-legal/litigation-legal/CLAUDE.md` → 外部律师指令风格、签署人默认值、预算立场。
3. 对每个在范围内的案件：读取 `matter.md` + `history.md`，起草逐案邮件（Markdown 格式）。
4. 输出 Markdown 到 `~/.claude/plugins/config/claude-for-legal/litigation-legal/oc-status/[YYYY-MM-DD]/[slug].md`。
5. 撰写总结。
...
- If  is NOT available or fails: fall back to markdown-only and tell the user
