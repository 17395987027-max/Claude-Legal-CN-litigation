...
Facts happen in order. The chronology is the spine every narrative hangs on — the statement of facts in a brief, reserve memos, settlement memos, 庭前准备, witness prep. Building a chron by hand is slow; AI is good at structured extraction. The catch: garbage-in, garbage-out. This skill pulls from the sources the configuration declares and from whatever the user uploads.
...
- **`--matter` mode (default for in-house litigation counsel).** Matter-history-focused. Reads the matter's case theory and key facts from `matter.md`, pulls from declared document-storage sources (本地文件系统 — whatever the `## Landscape` section of CLAUDE.md declares). Output is matter-centric: what happened across the dispute, tagged for advocacy use.
- **`--documents` mode (default for firm associate / paralegal).** Production-document-focused. Reads the case theory from the configuration, then extracts from user-provided document sets. Output is production-centric: what the documents show, tagged per the case theory.
...
- 文档集合 platform metadata if a connector is available (本地文件系统) — by custodian + date range.
- 本地文件-range manifest or production index if the user points at one.
...
3. **Declared sources** — the `Document storage` table in CLAUDE.md, filtered to ones this matter might touch (e.g., 本地邮件存档 for sender-side communications, SharePoint legal folder).
...
1. **Production export / 本地文件 set** — the user points at the production directory or a manifest; the skill reads by 本地文件 range + date.
2. **文档集合 connector** — if an MCP connector is available (本地文件系统), pull by custodian + date range.
...
- **Email archives (邮件客户端)** — if an MCP connector is authenticated, query by date range + counterparty / key terms; otherwise the user exports relevant threads to a folder.
- **文档集合 platforms (本地文件系统)** — if connector is available, pull by custodian + date range; otherwise the user provides an export.
...
|---|---|---|---|---|
| [YYYY-MM-DD] | [what happened, one sentence] | 🔴/🟡/⚪ | [blank / 🔒-flag / 🔒-review] | [file paths or 本地文件] |
...
**Unreadable sources:**
[sources declared in CLAUDE.md but not accessible this run — e.g., "云存储源 — 无 MCP 连接; 需手动导出"]
...
- `[CITE NEEDED: 本地文件 / exhibit / 庭审笔录 页码:行号]`
...
- **Guarantee completeness.** A chronology is only as good as the sources. If the 文档集合 production is ongoing and only 20% has landed, the chronology reflects that. Name the limitation.
