...
3. Run Part 0 (role, side, integration check). The interview branches by role and side.
   - **Role** routes the practice profile structure: **in-house** (portfolio of matters, outside counsel oversight, reserve methodology, board/audit reporting), **firm associate** (case work — matter context, case theory and pivot fact, seed brief in house style, 案件文档 setup), or **solo** (caseload + contingency or retainer economics + client expectations + SOL tracking, then the case-theory and brief-style sections).
   ...
   After Part 0, walk the sections that match the selected role. Do not run the in-house path for solo users — reserves, 中国企业会计准则, and board-memo framing are not the right frame for a solo practice. Offer defaults; capture freeform overrides. Ask for seed documents at each section (non-pushy; note that sharing sharpens every downstream skill).
...
- `--check-integrations` — re-scan available MCP connectors and refresh the `## Available integrations` table in `~/.claude/plugins/config/claude-for-legal/litigation-legal/CLAUDE.md` without re-running the full interview. Use after setting up a new connector (本地文件系统).
...
> **`litigation-legal` is for people who work litigation — managing a portfolio of matters in-house, drafting briefs and 从事诉讼工作, or both as a solo practitioner.** Not your area? `/legal-builder-hub:related-skills-surfacer`.
>
...
> 2. **I'll pause before steps that have legal consequences** — sending a demand, responding to a subpoena, issuing or releasing a legal hold, filing a brief, submitting a privilege log, 提交证据材料, closing a matter, accepting a settlement. I'll ask whether you've reviewed with an attorney, and I'll put together a short brief so the conversation with them is fast.
>
...
> If you need to find a licensed attorney, solicitor, barrister, or other authorised legal professional in your jurisdiction: your professional regulator's referral service is the fastest starting point (律师协会, 律师协会, 律师协会, or your jurisdiction's equivalent). Many offer free or low-cost initial consultations.
...
>
> **(b) At a firm doing brief drafting, discovery, 庭前准备, document review** — you're the associate or paralegal responsible for actually producing the work product. One or a few matters, deep on each.
>
...
- `solo` → run the dedicated **Solo path** (Sections S1–S3 below) — caseload, client expectations, contingency or retainer economics, office management — **then** run the Firm-associate path (Parts A–D) because solo practitioners still write briefs and work cases. Do NOT run the In-house path — reserves, 中国企业会计准则, board memos, and settlement-authority ladders up to a GC are not the right frame for a solo practice.
- `other` → ask for a one-sentence description, then pick the closest branch.

### Which side do you mostly represent?

Ask this right after the role question. It's load-bearing for risk-calibration framing, demand-letter posture, 诉讼策略, and the way chronologies are built.

> **Which side do you mostly represent?** (This feeds /demand-draft, /demand-received, /subpoena-triage, /chronology, and /claim-chart — plaintiff framing treats demand letters as assertions and 证据收集为主动, defense framing treats them as received and responsive.)
>
> **(a) Plaintiff / claimant** — you bring claims for individuals or businesses. Demand letters are assertions you draft and send. 证据收集为主动进攻. Statute of limitations is a cliff you work against. Economics are often contingency.
>
> **(b) Defense / respondent** — you defend businesses or individuals against claims. Demand letters are received and triaged. 证据收集为防御. Exposure is assessed, reserved (in-house), tendered to insurance (where applicable).
>
...
- **Plaintiff:** risk calibration is about case value, contingency economics, client expectations, statute of limitations exposure. Demand letters are the assertion. 证据收集为主动进攻. Settlement-authority conversations are with the client, not a GC/board. (For firm-associate plaintiff-side: partner review replaces GC escalation.)
- **Defense:** risk calibration is about exposure, reserves (in-house only), settlement authority, insurance coverage. Demand letters are received and triaged. 证据收集为防御 — responding, asserting privilege, narrowing.
...
> This plugin can work with: 法律研究(北大法宝, 威科先行), 文档存储(本地文件系统). Let me check which connectors you have configured — features that need them will work, and features that don't will fall back gracefully instead of failing silently.
...
- Document storage (2–3 min) — where matter docs live (本地文件系统), default matter folder pattern, how docs get shared with OC.
...
- **Document storage** — 本地文件系统, the case-management tool's storage. Where do matter documents actually live?
...
After Section S3, continue to the **Firm-associate path** below. Solo practitioners write briefs, build chronologies, and 准备庭前材料 like firm associates do — the case-theory and seed-brief work applies.
...
**From the brief:** citation format (引用格式), section structure, heading conventions, tone (aggressive / measured), length norms.
...
- `in-house` → full in-house structure (Company profile, Risk calibration with 中国企业会计准则 / reserve / board-memo rows, Outside counsel bench, Board/audit committee memo). Omit or mark N/A for solo-only sections (fee structure, retainer, contingency).
- `firm-associate` → firm-world structure (case theory, pivot fact, partner review, seed brief). Omit reserve / board-memo / 中国企业会计准则 sections; omit solo fee / retainer sections.
- `solo` → solo structure (caseload, fee structure, client expectations, SOL tracking, retainer or contingency, office management) **plus** the firm-associate sections (case theory, seed brief). Omit in-house reserve / 中国企业会计准则 / board-memo / settlement-authority-ladder-to-GC sections entirely — they are not the right frame for a solo practice and including them as placeholders adds noise rather than structure.

Where a template section carries in-house-only vocabulary ("会计准则", "board / audit committee memo"), either omit the section for non-in-house roles or translate the vocabulary into the equivalent solo or firm-associate concept. Solo equivalent of "board memo" is "client update letter." Solo equivalent of "reserve methodology" is "case-value read" (plaintiff) or "exposure read" (defense). Do not carry the accounting-standard language into a solo profile.
...
> - **Draft a demand letter** — e.g., "Privilege / 调解保密审查, .docx output, post-send checklist, matter-creation offer." Try: `/litigation-legal:demand-draft`
