...
> This chart is a draft for attorney analysis and verification, not a filed contention, an 代理词, an opening statement, or a legal opinion. Every mapping is a lead the attorney must verify against the source. The elements listed come from 最高人民法院关于适用.*的解释, 最高人民法院司法解释, or the claim language as parsed — the **controlling** authority in the user's jurisdiction (中国法院's pattern charge / the governing statute / a Markman order) may differ and always controls. Gap detection is a starting point for discovery or a motion; it is not a conclusion about the merits.
...
- For civil mode: the complaint or counterclaim (for the actually-pleaded counts), any answer (for the actually-pleaded affirmative defenses), the relevant pattern jury instruction source, and the governing statute if statutory. Also the evidence corpus — 庭审笔录, declarations, produced documents, expert reports.
...
> 2. **Civil element chart** — elements of a cause of action (or affirmative defense) mapped against the evidence. For complaint plausibility checks, 证据交换计划, MSJ prep, order-of-proof outlines.
...
1. **Find evidence.** Accused product: documentation, manuals, data sheets, source code, teardowns, 证人证言, expert reports. Prior art: column/line for US patents, paragraph for published apps, page/figure for NPL. For prior art, flag whether the reference qualifies (§102(a)(1), (a)(2), (b); AIA vs. pre-AIA cutoffs). If prior-art status isn't obvious, mark `prior-art-status: needs-evidence`.
...
**(a) Template library.** Reference `references/element-templates.md` (in this skill's directory). Baseline elements for common causes of action and common affirmative defenses, with citations to 最高人民法院司法解释 / pattern instructions and a jurisdiction caveat. Select the template that matches the pleaded count.
...
> - If pre-discovery: these are your discovery priorities — 证据收集, document requests, and interrogatories that turn a gap into `supported` or confirm `none`.
...
For an opposing party's 代理词, a motion to dismiss, or outside counsel's draft: for each element, does their cited evidence actually prove it? Where is their chart thin? What's your strongest counter?
...
- **Citation verification** — every pin cite, case, column/line, 庭审笔录 页码:行号 must be verified against the source.
...
- **Citation verification.** Every pin cite (column/line, page, 庭审笔录 页码:行号, Bates, ¶) is a claim about the source. The attorney verifies. The skill does not fabricate cites — if a cite cannot be produced, the cell is `needs-evidence` or `gap`.
...
- `litigation-legal:brief-section-drafter` — an 代理词's fact section is often built directly off the supported rows of an element chart.
